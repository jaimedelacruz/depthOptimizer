...
namespace mem{
  ...
  constexpr const size_t VNAME_LENGTH = 15;
  ...
  public:
    ...
    M_INLINE const T* getDataBlock()const{return Data.getData();}
    ...
    M_INLINE Array(Array<T,N> const &in): 
      Dimensions(in.getDimensions()),Data(){Data = in.getStorage();}
    ...
    M_INLINE explicit Array(std::array<idx_t,N> const& indim):
      Dimensions(indim), Data(Dimensions.size()){};
    ...
    // ------------------------------------------------------------------ //
    template<typename ...S>
    void Map(T* din_, const S... indexes){
      static_assert((sizeof...(S) == N || sizeof...(S) == 2*N));
      internal::DDim<idx_t, N> const nDim(indexes...);
      
      Data.set(internal::Storage<T>(nDim.size(), din_));
