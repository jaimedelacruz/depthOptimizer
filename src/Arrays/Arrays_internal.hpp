...
namespace mem{
  namespace internal{
	...
	n_elements(0), data(), allocated(false){*this = istor;}
	...
	data = NULL;
      ...
      M_INLINE void set(Storage<T> const& istor){
	if(allocated){
	  amem::aligned_free<T>(data);
	  allocated = false;
	  data = NULL;
	...
	n_elements = istor.size();
	data = istor.getData();
	allocated = istor.allocated;
      }
